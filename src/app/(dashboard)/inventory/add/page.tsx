...
const initialEquipmentState: Omit<Equipment, 'id'> = {
    internalId: '',
    name: '',
    model: '',
    brand: '',
...
const documentationItems: { id: keyof Equipment['requiredDocumentation'], label: string }[] = [
    { id: 'invima', label: 'Registro Sanitario' },
    { id: 'importDeclaration', label: 'Declaración Importación' },
    { id: 'userManual', label: 'Manual de Usuario' },
    { id: 'serviceManual', label: 'Manual de Servicio' },
    { id: 'quickGuide', label: 'Guía Rápida' },
    { id: 'other', label: 'Otro' },
];
...
export default function EquipmentAddPage() {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(initialEquipmentState.imageUrl);
  ...
  const handleSave = (event: React.FormEvent) => {
    ...
    if (equipment) {
        ...
        addEquipment(processedEquipment);
        toast({
            title: "¡Equipo Añadido!",
            description: `${equipment.name} ha sido añadido al inventario.`,
  ...
  const handleCheckboxChange = (id: keyof Equipment['requiredDocumentation'], checked: boolean) => {
    setEquipment(prev => {
        ...
        // If 'other' is unchecked, clear the detail field
        const newOtherDetail = id === 'other' && !checked ? '' : prev.otherDocumentationDetail;

        return {
            ...prev,
            requiredDocumentation: newDocStatus,
            otherDocumentationDetail: newOtherDetail,
        };
    });
  };
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setEquipment(prev => ({ ...prev, imageUrl: result }));
      };
      ...
      <div className="flex items-center gap-4">
       ...
       <ClientOnly>
        <form className="grid gap-6" onSubmit={handleSave}>
              <div className="grid auto-rows-max items-start gap-6 lg:grid-cols-3 lg:gap-8">
                  <div className="grid auto-rows-max items-start gap-6 lg:col-span-2">
                     <Card>
                      <CardHeader>
                          <CardTitle>Información Principal</CardTitle>
                          <CardDescription>
                              Detalles básicos, de identificación, ubicación y estado del equipo.
                          </CardDescription>
                      </CardHeader>
                      <CardContent className="grid gap-6">
                           <div className="grid md:grid-cols-2 gap-4">
                              <div className="grid gap-2">
                                  ...
                                  <Input id="name" value={equipment.name} onChange={handleInputChange} required />
                              </div>
                              <div className="grid gap-2">
                                  <Label htmlFor="brand" className="flex items-center gap-2"><Building />Marca</Label>
                                  <Input id="brand" value={equipment.brand} onChange={handleInputChange} required/>
                              </div>
                          </div>
                           <div className="grid md:grid-cols-2 gap-4">
                              <div className="grid gap-2">
                                  <Label htmlFor="model" className="flex items-center gap-2"><Puzzle />Modelo</Label>
                                  <Input id="model" value={equipment.model} onChange={handleInputChange} required/>
                              </div>
                              <div className="grid gap-2">
                                  <Label htmlFor="serial" className="flex items-center gap-2"><Barcode />Serie</Label>
                                  <Input id="serial" value={equipment.serial} onChange={handleInputChange} required/>
                              </div>
                          </div>
                           <div className="grid md:grid-cols-2 gap-4">
                               <div className="grid gap-2">
                              ...
                              <div className="grid gap-2">
                                  ...
                                  <Input id="invima" value={equipment.invima} onChange={handleInputChange} required/>
                              </div>
                           </div>
                           <div className="grid md:grid-cols-2 gap-4">
                              <div className="grid gap-2">
                               ...
                               <div className="grid gap-2">
                                  ...
                                  <Input id="location" value={equipment.location} onChange={handleInputChange} required/>
                              </div>
                          </div>
                          ...
                          <div className="grid md:grid-cols-3 gap-4">
                              <div className="grid gap-2">
                                  ...
                                  <Select value={equipment.risk} onValueChange={(value) => handleSelectChange('risk', value as Equipment['risk'])}>
                              ...
                              <div className="grid gap-2">
                                  ...
                                  <Select value={equipment.classificationByUse} onValueChange={(value) => handleSelectChange('classificationByUse', value)}>
                                      <SelectTrigger id="classificationByUse">
                                          <SelectValue />
                                      </SelectTrigger>
                                      <SelectContent>
                      ...
                      <Card>
                          ...
                          <CardContent className="grid grid-cols-2 gap-6 pt-2">
                               ...
                               <div className="grid gap-2">
                                  <Label htmlFor="installationDate" className="flex items-center gap-2"><Calendar />Fecha de Instalación</Label>
                   ...
                   <div className="grid auto-rows-max items-start gap-6 lg:gap-8">
                      <Card className="overflow-hidden">
                          ...
                          <CardContent className="space-y-4">
                              <div className="relative group aspect-video w-full overflow-hidden rounded-md border-2 border-dashed flex items-center justify-center">
                                  {imagePreview ? (
                                      <div className="relative w-full h-full">
                                        <Image 
                                            src={imagePreview}
                                            alt={`Imagen de ${equipment.name || 'equipo nuevo'}`}
                                            fill
                                      ...
                                      <div className="text-center text-muted-foreground p-4">
                                          ...
                                          <p className="mt-2 text-sm">Sube una imagen</p>
                                      </div>
                                  )}
                                   <div 
                       ...
                       <Card>
                        ...
                        <CardContent className="grid grid-cols-2 gap-x-8 gap-y-4 pt-4">
                            {documentationItems.map(doc => (
                                <div key={doc.id} className="flex items-center gap-2">
                                    <Checkbox
                                        id={`doc-${doc.id}`}
                                        checked={equipment.requiredDocumentation[doc.id as keyof typeof equipment.requiredDocumentation]}
                                        onCheckedChange={(checked) => handleCheckboxChange(doc.id as keyof typeof equipment.requiredDocumentation, checked as boolean)}
                                    />
                                    <Label htmlFor={`doc-${doc.id}`} className="font-normal">{doc.label}</Label>
                                </div>
                            ))}
                        </CardContent>
                        {equipment.requiredDocumentation.other && (
                            <CardContent className="pt-0">
                                <div className="grid gap-2">
                                    <Label htmlFor="otherDocumentationDetail">Especificar Otro</Label>
                                    <Input 
